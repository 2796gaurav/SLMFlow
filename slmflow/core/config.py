...
class SLMConfig(BaseModel):
    ...
    load_in_4bit: bool = Field(
        default=True,
        description="Load model in 4-bit quantization (QLoRA)"
    )
...
class TrainingConfig(BaseModel):
    ...
    max_steps: int = Field(
        default=500,
        ge=1,
        description="Maximum training steps (optimized for T4)"
    )
    num_epochs: Optional[float] = Field(
        default=None,
        description="Number of epochs (overrides max_steps if set)"
    )
    ...
    warmup_ratio: Optional[float] = Field(
        default=None,
        description="Warmup ratio (overrides warmup_steps if set)"
    )
    ...
    use_lora: bool = Field(
        default=True,
        description="Use LoRA for parameter-efficient training"
    )
    lora_r: int = Field(
        default=16,
        ge=1,
    ...
    optim: str = Field(
        default="adamw_8bit",
        description="Optimizer (adamw_8bit for memory efficiency)"
    )
    ...
    report_to: list[str] = Field(
        ...
        description="Reporting integrations"
    )
    ...
    bf16: bool = Field(
        default=False,
        description="Use BF16 mixed precision (for A100+)"
    )
...
class DistillationConfig(BaseModel):
    ...
    teacher_model: str = Field(
        description="Teacher model HuggingFace ID or path"
    )
    teacher_dtype: DType = Field(
        default=DType.FLOAT16,
        description="Teacher model precision"
    )
    teacher_load_in_4bit: bool = Field(
        default=True,
        description="Load teacher in 4-bit for memory efficiency"
    )
    ...
    temperature: float = Field(
        ...
        ge=1.0,
        le=10.0,
        description="Distillation temperature (higher = softer)"
    )
    alpha: float = Field(
        default=0.5,
        ge=0.0,
    ...
    progressive: bool = Field(
        default=False,
        description="Use progressive distillation (layer-by-layer)"
    )
    
    # Attention transfer
    attention_transfer: bool = Field(
        default=False,
        description="Transfer attention patterns from teacher"
    )
...
class InferenceConfig(BaseModel):
    ...
    quantization: Optional[Literal["awq", "gptq", "int8", "int4"]] = Field(
        default=None,
        description="Quantization method for deployment"
    )
