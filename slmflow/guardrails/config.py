...
class GuardrailsConfig(BaseModel):
    ...
    pii_action: str = Field(
        default="redact",
        description="Action for PII: 'redact', 'mask', or 'block'"
    )
    
    # Toxicity Filter
    enable_toxicity_filter: bool = Field(
        default=True,
        description="Enable toxicity detection and filtering"
    )
    toxicity_threshold: float = Field(
        default=0.7,
        ge=0.0,
        le=1.0,
        description="Toxicity score threshold for filtering"
    )
    ...
    blocked_phrases: list[str] = Field(
        ...
        description="Custom phrases to block"
    )
    ...
    log_violations: bool = Field(
        default=True,
        description="Log guardrail violations"
    )
